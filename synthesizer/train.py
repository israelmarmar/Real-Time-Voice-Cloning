...
def train(run_id: str, syn_dir: Path, models_dir: Path, save_every: int,  backup_every: int, force_restart: bool,
    ...
    if os.path.isfile(history_train):
        ...
        # Bookkeeping
        time_window = history_data["time_window"]
        loss_window = history_data["loss_window"]

        model = history_data["model"]
        dataset = history_data["dataset"]
        data_loader = history_data["data_loader"]
        epochs = history_data["epochs"]
        last_epoch = history_data["last_epoch"]
        last_step_in_epoch = history_data["last_step_in_epoch"]

        # Initialize the optimizer
        optimizer = optim.Adam(model.parameters())

        for session in hparams.tts_schedule:
            _, _, max_step, batch_size = session
    ...
    for epoch in range(last_epoch, epochs+1):
        ...
        for i, (texts, mels, embeds, idx) in enumerate(data_loader, 1):
            ...
            if i < last_step_in_epoch + 1:
                continue

            last_step_in_epoch = i
            ...
            history_data = {
                ...
                "epochs": epochs,
                "last_epoch": last_epoch_var,
                "last_step_in_epoch": last_step_in_epoch,
